...
This report is generated by an AI-powered analysis system (YOLO + Depth Estimation) and should be reviewed by qualified personnel.
  ...
  // Effect to handle custom camera trigger from BottomNav
  useEffect(() => {
    const handleTrigger = () => {
      handleCameraCapture()
    }
    ...
    // Check query param for trigger
    if (typeof window !== "undefined") {
      const urlParams = new URLSearchParams(window.location.search)
      if (urlParams.get("trigger") === "camera") {
        // Clean up URL
        window.history.replaceState({}, "", "/dashboard")
        // Small delay to ensure render
        setTimeout(() => handleCameraCapture(), 500)
      }
    }

    return () => {
      window.removeEventListener("trigger-camera", handleTrigger)
    }
  }, [handleCameraCapture])
