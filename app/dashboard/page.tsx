...
export default function BillboardDetectionApp() {
  ...
  const handleMLComplete = async (mlData: any) => {
    ...
    await saveToBackend(result, mlData)
  }

  const saveToBackend = async (result: AnalysisResult, mlData: any) => {
    ...
    try {
      ...
      alert("Analysis complete and report saved to database!")
  ...
  // Legacy manual submit (kept for fallback but auto-save is preferred now)
  const submitViolationReport = async () => {
    // This is now redundant if we auto-save, but keeping logic for manual override if needed
    // or we can remove it. For now, let's just alert.
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-8 space-y-8">
        ...
        {/* Map Section */}
        <section>
          <DashboardMap billboards={billboards} />
        </section>
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="flex gap-2 flex-wrap">
                    ...
                    {/* Save button removed as it auto-saves now, but kept generate report */}
                  </div>
                </CardContent>
              </Card>
            )}
