...
export default function BillboardDetectionApp() {
  ...
  const saveToBackend = async (result: AnalysisResult) => {
    ...
    try {
      ...
      // Add unique request ID to prevent duplicate submissions
      const requestId = result.id || `req-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
              ...
              <CardContent className="space-y-4">
                ...
                {!previewUrl ? (
                  <div className="space-y-4">
                    ...
                    {/* Mobile Only: Floating Action Button (FAB) at Bottom Middle */}
                    <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-50 md:hidden print:hidden animate-in fade-in slide-in-from-bottom duration-500">
                      <Button
                        onClick={(e) => {
                          e.stopPropagation();
                          handleCameraCapture();
                        }}
                        className="h-16 w-16 rounded-full shadow-[0_0_20px_rgba(var(--primary),0.5)] border-2 border-white/20 bg-gradient-to-t from-primary to-primary/80 animate-pulse-glow flex items-center justify-center backdrop-blur-md"
                        size="icon"
                      >
                        <Camera className="h-8 w-8 text-primary-foreground drop-shadow-md" />
                        <span className="sr-only">Take Photo</span>
                      </Button>
                      <div className="absolute -bottom-8 left-1/2 -translate-x-1/2 text-[10px] font-medium text-muted-foreground whitespace-nowrap bg-background/80 backdrop-blur-sm px-2 py-0.5 rounded-full border border-border/50 shadow-sm mt-1">
                        Tap to Scan
                      </div>
                    </div>
                  ...
                  <div className="space-y-4">
                    ...
                    {!isAnalyzing && !analysisResult && (
                      <Button
                        ...
                        className="w-full shadow-md hover:shadow-glow transition-all duration-300 animate-pulse-glow"
          ...
          <div className="space-y-6">
            ...
            {analysisResult && (
              <Card>
                ...
                <CardContent className="space-y-4">
                  {/* Analyzed Image */}
                  <div className="rounded-lg overflow-hidden border h-64 w-full bg-muted/20 relative">
                    {/* Use standard img for base64 or external URLs to avoid Next.js Image config issues with dynamic sources */}
                    <img
